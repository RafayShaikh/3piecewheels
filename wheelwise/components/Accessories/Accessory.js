...
function Accessory({ id, item }) {
  ...
  const deleteEntry = async (docId) => {
    ...
    const ref = db.collection('accessories').doc(docId);
    const data = await ref.get();

    setDeleteState(data.data());

    const collectionRef = await db.collection('accessories');
    collectionRef
      .doc(docId)
      .delete()
      .then(() => {
  ...
  const handleClick = (e) => {
    router.push({
      pathname: `/accessories/${JSON.stringify(id)}`,
      query: {
